    ...
    <div className="flex h-screen overflow-hidden bg-background relative">
      ...
      >
        ...
        <nav className={`flex-1 py-2 space-y-1 overflow-y-auto overscroll-contain ${sidebarCollapsed ? "px-2" : "px-3"}`}>
          ...
          {/* Secondary nav — visible on desktop, hidden on mobile (mobile uses tab bar) */}
          {!sidebarCollapsed && (
            <div className="pt-2 mt-2 border-t border-border/30">
              <p className="px-3 py-1 text-[9px] text-muted-foreground/40 uppercase tracking-widest font-medium">More</p>
            </div>
          )}
            ...
            return (
              <Link key={item.href} href={item.href}>
                <div
                  title={sidebarCollapsed ? item.label : undefined}
                  className={`flex items-center rounded-xl cursor-pointer transition-all duration-150 ${
                    sidebarCollapsed ? "justify-center p-2.5" : "gap-3 px-3 py-1.5 text-xs"
                  } ${
                    isActive
